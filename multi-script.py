"""
    ...
    Workflow:
    This script watch stories, reply pending messages, 
    reply messages and like image comments All in one.
    All functions are running on different threads at
    the same time.
"""
import argparse
import threading
        ...
        try:
            for w in json["story"]["items"]:
                ...
                if bot.api.see_reels(w) == False:
                    print(bot.api.see_reels(w))
                    print("Falling asleep wait 200 seconds")
...
def like_self_media_comments():
    ...
    while True:
        try:
            ...
            comment_id = bot.api.last_json["items"][x]["caption"]
            bot.like_media_comments(item)
            bot.api.like_comment(comment_id)
